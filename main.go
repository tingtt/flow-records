...
func main() {
	...
	e.PATCH("/:id", patch)
	e.DELETE("/:id", delete)
	gChangeLogs := e.Group("/changelogs")
	gChangeLogs.POST("/", changeLogPost)
	gChangeLogs.GET("/", changeLogGetList)
	gChangeLogs.GET("/:id", changeLogGet)
	gChangeLogs.PATCH("/:id", changeLogPatch)
	gChangeLogs.DELETE("/:id", changeLogDelete)
